			...
			jobButton.DoClick = function( self )
				if type(v.model) == "table" and table.Count( v.model ) > 1 then
					...
					CH_MAIN.Paint = function( self, w, h )
						...
						draw.SimpleText( "Choisissez Votre Apparence", "RP_SubFontThick", 24, 20, Color( 190, 190, 190 ) )
					...
					CH_OKAY.DoClick = function()
						...
						if v.vote then
								if ((v.admin == 0 and LocalPlayer():IsAdmin()) or (v.admin == 1 and LocalPlayer():IsSuperAdmin())) then
										...
										menu:AddOption("Voter", function() RunCmd("/vote"..v.command)
											BG_PANEL:Remove()
											CH_MAIN:Close()
											CH_BG:Remove()
										end)
										menu:AddOption("Ne Pas Voter", function() RunCmd("/"..v.command)
											BG_PANEL:Remove()
											CH_MAIN:Close()
											CH_BG:Remove()
										end)
					...
					if v.vote then
						if ((v.admin == 0 and LocalPlayer():IsAdmin()) or (v.admin == 1 and LocalPlayer():IsSuperAdmin())) then
								...
								menu:AddOption("Voter", function() RunCmd("/vote"..v.command) BG_PANEL:Remove() end)
								menu:AddOption("Ne Pas Voter", function() RunCmd("/"..v.command) BG_PANEL:Remove() end)
								menu:Open()
						else
								RunCmd("/vote"..v.command)
								BG_PANEL:Remove()
...
function LoadCmdPanel()
	...
	JOB_SIDE.Paint = function( self, w, h )
		...
		draw.SimpleText( "Emplois", "RP_SubFontThick", 24, 20, Color( 190, 190, 190 ) )
	...
	if ELITE.VIPJobCheckbox then
		...
		VIP_CHECK:SetText( "Afficher Les Emplois Pour Destiny" )
