function OpenWeaponsTab()
	...
	WEP_WINDOW.Paint = function( self, w, h )
		...
		if not weapons then
			draw.SimpleText( "Rien N'est Disponible!", "RP_SubFontThick", WEP_WINDOW:GetWide() / 2, WEP_WINDOW:GetTall() / 2, Color( 190, 190, 190 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		end
	end
	...
	SHIP_WINDOW.Paint = function( self, w, h )
		...
		draw.SimpleText( "Caisse", "RP_SubFontThick", 24, 20, Color( 190, 190, 190 ) )
		
		surface.SetDrawColor( Color( 242, 242, 242 ) )
		surface.DrawLine( 24, 44, w - 26, 44 )
		
		if not shipments then
			draw.SimpleText( "Rien N'est Disponible!", "RP_SubFontThick", SHIP_WINDOW:GetWide() / 2, SHIP_WINDOW:GetTall() / 2, Color( 190, 190, 190 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		end
	end
	...
	for k, v in pairs( CustomShipments ) do
		...
		if (v.seperate and (not GAMEMODE.Config.restrictbuypistol or
			...
			entpan.Paint = function( self, w, h )
				...
				draw.SimpleText( "Prix: $"..string.upper(v.pricesep), "RP_SubFontThick", 71, 20, Color( 210, 210, 210 ) )
			end
	...
	for k, v in pairs( CustomShipments ) do
		...
		if not v.noship and table.HasValue(v.allowed, LocalPlayer():Team())
			...
			entpan.Paint = function( self, w, h )
				...
				surface.DrawLine( 0, h - 1, w, h - 1 )
				draw.SimpleText( string.upper(v.name), "RP_SubFontThick", 70, 6, Color( 0, 0, 0 ) )
				draw.SimpleText( "Prix: $"..string.upper(v.price), "RP_SubFontThick", 71, 20, Color( 210, 210, 210 ) )
